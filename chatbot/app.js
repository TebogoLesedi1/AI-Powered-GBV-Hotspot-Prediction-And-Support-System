...
document.querySelector('#voice-input').addEventListener('click', () => { const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition; if (!Recognition) return; const recognition = new Recognition(); recognition.lang = languageSelect.value; recognition.onresult = event => { question.value = event.results[0][0].transcript; submit(question.value); }; recognition.start(); });
languageSelect.addEventListener('change', () => { if (languageSelect.value !== 'en-ZA') document.querySelector('#record-count').textContent = safetyPhrases[languageSelect.value] || 'Translation is available when you read an answer aloud.'; });
loadReport();